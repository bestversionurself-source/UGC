...
api_guard(function (): void {
    ...
    $payload = json_encode(['amount' => $plans[$plan]['amount'], 'currency' => 'INR', 'receipt' => 'ugc_' . $userId . '_' . time(), 'notes' => ['user_id' => (string)$userId, 'package' => $plan]]);
    ...
    db()->prepare('INSERT INTO payments (user_id, provider_order_id, plan, amount_paise) VALUES (?, ?, ?, ?)')->execute([$userId, $order['id'], $plans[$plan]['account_plan'], $plans[$plan]['amount']]);
    respond(['key' => $key, 'order_id' => $order['id'], 'amount' => $plans[$plan]['amount'], 'currency' => 'INR', 'name' => safe_user($userId)['name']]);
});
